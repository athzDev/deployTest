...
import { getCustomerAddressInfo, getCustomerById, getCustomerExtras, getCustomerRemarks, getCustomersList, loadMaster, saveCustomers } from 'src/config/api'
...
type CustomerInnerDetailsProps = {
    partner_Id: string,
    partnerName: string,
    partnerCode1: string,
    credit: string,
...
const EditCustomer = () => {
    ...
    const [customerDetails, setCustomerDetails] = useState({} as CustomerInnerDetailsProps)
    ...
    useEffect(() => {
        prefilledCustomerDetail()
    }, [])

    
    //prefill data
    const prefilledCustomerDetail = () => {
        getCustomerById(id).then((response) => {
        const data = response.data
        setCustomerDetails(data[0]);
        setCustomerState(prevProp => ({
            ...prevProp,
            partner_Id: data[0].partner_Id,
            partnerName: data[0].partnerName,
            partnerCode1: data[0].partnerCode1,
            crAmt: data[0].crAmt,
            telephone: data[0].telephone,
            whatsappNo: data[0].whatsappNo,
            fax: data[0].fax,
            email1: data[0].email1,
            website: data[0].website,
            billName: data[0].billName,
            partnerType1_FK:data[0].partnerType1_FK,
            partnerGroup1_FK: data[0].partnerGroup1_FK
      }))
    ...
    const handleTabChange = (e, title, index) => {
        ...
        if (title === "Address Information"){
            getCustomerAddressInfo(id).then((res) => {
                const data = res.data
                if(data.length !== 0){
                    setCustomerAddressState(prevProp => ({
                        ...prevProp,
                        block: data[0].block,
                ...
                setCustomerAddressState(data);
                if (res.data.success === false) {
                  toast.error(res.data.data.status);
                }
            })
            .catch((err) => toast.error("Something Went Wrong, Please Try Again"));
        } else if (title === "Extra Information"){
                getCustomerExtras(id).then((res) => {
                    const data = res.data
                    if(data.length !== 0){
                    setCustomerState(prevProp => ({
                        ...prevProp,
                            currency: data[0].currency,
                            ddlPaymentTerms: data[0].ddlPaymentTerms,
                            ddlTaxingScheme: data[0].ddlTaxingScheme,
                            defaultSalesRep: data[0].defaultSalesRep,
                            ddlbranch: data[0].ddlbranch,
                            ddlCarrier: data[0].ddlCarrier,
                            discountVal: data[0].discountVal,
                  }))
                }
                    if (res.data.success === false) {
                      toast.error(res.data.data.status);
                ...
                .catch((err) => toast.error("Something Went Wrong, Please Try Again"));
    } else if (title === "Remark Labels"){
        getCustomerRemarks(id).then((res) => {
            const data = res.data
            if(data.length !== 0){
            setCustomerState(prevProp => ({
                ...prevProp,
                remarks: data[0].remarks,
                remarks2: data[0].remarks2        
          }))
        }
            if (res.data.success === false) {
              toast.error(res.data.data.status);
                                        ...
                                        <FormFieldWrapper>
                                            ...
                                            <MasterFormSecButtonsWrap>
                                                <MasterFormSecButtonsLeft>
                                                    <PosBlackBtn onClick={() => setFilterType("Address Information")}>Next</PosBlackBtn>
                                                </MasterFormSecButtonsLeft>
                                    ...
                                    <>
                                        ...
                                        <MasterFormSecBody>
                                            ...
                                            <FormFieldWrapper>
                                                ...
                                                <MasterFormSecButtonsWrap>
                                                    <MasterFormSecButtonsLeft>
                                                        <PosBlackBtn onClick={() => setFilterType("Personal Info")}>Previous</PosBlackBtn>
                                                        <PosBlackBtn onClick={() => setFilterType("Extra Information")}>Next</PosBlackBtn>
                                                    </MasterFormSecButtonsLeft>
                                                    <MasterFormSecButtonsRight>
                                                        <PosTransparentBtn onClick={resetCustomerDetail}>Discard Changes</PosTransparentBtn>
                                                        <PosBtn>Save</PosBtn>
                                        ...
                                        <>
                                            ...
                                            <MasterFormSecBody>
                                                ...
                                                <FormFieldWrapper>
                                                    <CustomerInputWrap>
                                                        ...
                                                        {/* <InputWrap>
                                                            ...
                                                            <select id='currency' placeholder='Select Currency' onChange={changeHandler} value="" name={"currency"}>
                                                                ...
                                                                {customerCurrency.map((currencies) => (
                                                                   <option value={currencies.master_ID}>{currencies.masterName}</option>
                                                                ))}
                                                            </select>
                                                        </InputWrap> */}
                                                    ...
                                                    <MasterFormSecButtonsWrap>
                                                        <MasterFormSecButtonsLeft>
                                                            <PosBlackBtn onClick={() => setFilterType("Address Information")}>Previous</PosBlackBtn>
                                            ...
                                            <>
                                                ...
                                                <MasterFormSecBody>
                                                    ...
                                                    <FormFieldWrapper>
                                                        ...
                                                        <MasterFormSecButtonsWrap>
                                                            <MasterFormSecButtonsLeft>
                                                                <PosBlackBtn onClick={() => setFilterType("Extra Information")}>Previous</PosBlackBtn>
                                                                <PosBlackBtn onClick={() => setFilterType("Remark Labels")}>Next</PosBlackBtn>
                                                            </MasterFormSecButtonsLeft>
                                                            <MasterFormSecButtonsRight>
                                                                <PosTransparentBtn onClick={resetCustomerDetail}>Discard Changes</PosTransparentBtn>
                                                                <PosBtn>Save</PosBtn>
                                        ...
                                        filterType === "Remark Labels" ?
                                                ...
                                                <>
                                                    ...
                                                    <MasterFormSecBody>
                                                        ...
                                                        <FormFieldWrapper>
                                                            ...
                                                            <MasterFormSecButtonsWrap>
                                                                <MasterFormSecButtonsLeft>
                                                                    <PosBlackBtn onClick={() => setFilterType("Account Information")}>Previous</PosBlackBtn>
                                                                    <PosBlackBtn onClick={() => setFilterType("Order History")}>Next</PosBlackBtn>
                                                                </MasterFormSecButtonsLeft>
                                                                <MasterFormSecButtonsRight>
                                            ...
                                            filterType === "Order History" ?
                                                <MasterFormSecWrapper>
                                                    ...
                                                    <MasterFormSecButtonsWrap>
                                                        <MasterFormSecButtonsLeft>
                                                            <PosBlackBtn onClick={() => setFilterType("Remark Labels")}>Previous</PosBlackBtn>
                                                ...
                                                filterType === "Payment History" ?
                                                    <MasterFormSecWrapper>
                                                        ...
                                                        <MasterFormSecButtonsWrap>
                                                            <MasterFormSecButtonsLeft>
                                                                <PosBlackBtn onClick={() => setFilterType("Order History")}>Previous</PosBlackBtn>
                                                                <PosBlackBtn onClick={() => setFilterType("Contracts")}>Next</PosBlackBtn>
                                                            </MasterFormSecButtonsLeft>
                                                            <MasterFormSecButtonsRight>
                                                                <PosTransparentBtn>Discard Changes</PosTransparentBtn>
                                                                <PosBtn>Save</PosBtn>
                                                    ...
                                                    filterType === "Contracts" ?
                                                        <MasterFormSecWrapper>
                                                            ...
                                                            <MasterFormSecButtonsWrap>
                                                                <MasterFormSecButtonsLeft>
                                                                    <PosBlackBtn onClick={() => setFilterType("Payment History")}>Previous</PosBlackBtn>
                                                                </MasterFormSecButtonsLeft>
                                                                <MasterFormSecButtonsRight>
                                                                    <PosTransparentBtn>Discard Changes</PosTransparentBtn>
                                                                    <PosBtn>Save</PosBtn>
...
export const CustomerInputWrap = styled.div`
  display: flex;
  width: 100%;
  gap: 10px;
`